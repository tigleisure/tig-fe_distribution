...
import BusCard from '@components/reservation/BusCard';
...
export default function Page({ params }: { params: { companyId: string } }) {
  ...
  if (isSuccess && data?.result.category !== 'BUS') {
    router.replace('/');
  }
  ...
  return (
    <main className="w-full h-full overflow-y-scroll flex flex-col ">
      ...
      {/* 시간 선택 카드는 운영시간 제거로 숨김 */}
      {/* <ResPeopleCountDropboxCard /> */}
      {/* <GameTypeCard /> */}
      <BusCard prices={(data?.result.prices as any) || []} number={3} />
