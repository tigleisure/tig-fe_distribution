...
import CateringCard from '@components/reservation/CateringCard';
...
export default function Page({ params }: { params: { companyId: string } }) {
  ...
  if (isSuccess && data?.result.category !== 'CATERING') {
    router.replace('/');
  }
  ...
  return (
    <main className="w-full h-full overflow-y-scroll flex flex-col ">
      ...
      <AddressCard />
      <ResPeopleCountDropboxCard number={3} />
      <CateringCard prices={(data?.result.prices as any) || []} number={4} from="package" />
      <RequestCard number={5} />
      <MakeResButtonCard
        clubName={clubName}
        address={address}
        clubStartTime="10:00"
